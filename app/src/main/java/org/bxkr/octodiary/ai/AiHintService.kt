...
/**
 * ИИ помощник для подсказок (без авторешения)
 */
object AiHintService {
    
    /**
     * Получить подсказку от ИИ по скриншоту
     */
    suspend fun getHint(
        context: Context,
        url: String,
    ...
    private fun buildHintPrompt(url: String): String {
        return """
            Ты - ИИ помощник для решения заданий. Твоя задача - дать ПОДСКАЗКУ, а не решить полностью.
            
            URL открытой страницы: $url
            ...
            **Твоя задача**:
            ...
            5. Намекни на первый шаг решения и дай "толчок", от которого можно будет оттолкнуться и пойти "по течению"
