...
*   **ВАЖНОЕ ПРАВИЛО:** После выполнения каждого пункта или подпункта необходимо добавлять в файл с подробным планом действий, если он есть, секцию **[REVIEW]**, описывающую конкретные изменения, внесенные в кодовую базу (какие файлы изменены, что добавлено/удалено, какие проблемы решены).
*   **План действий:** Если в репозитории нет файла с планом твоих действий на будущее, **создай** файл _plan.md_, и в него занеси план действий на будущее, параллельно соблюдая правило, которое я писал выше

## АЛГОРИТМ РАБОТЫ (THE "WORK" PROTOCOL)

При возникновении проблем, следуй этому алгоритму (основано на `работа.jpg`):

### 1. Запуск `ui_walker.py` и API
*   **Запуск:** Запусти `ai_walker.py` (с флагами `--api_key`, `--model`, `--api_base`).
*   **API отвечает успешно?**
    *   **ДА:** Переходи к блоку 2 (Анализ проблем).
    *   **НЕТ:**
        *   **Кончилась квота / Ошибки 401/402/429?**
            *   **НЕТ (Другая ошибка, цикл):** Завис в цикле? -> Да (первый раз) -> Retry. Да (3+ раз) -> Смени модель.
            *   **ДА (Квота):** Проверь наличие локального AI (LM Studio/ollama).
                *   *Есть:* Проверь ресурсы (`df -h`, RAM/VRAM). Есть место (1-1.5GB RAM + 0.5GB VRAM запас)?
                    *   **Да:** Загрузи модель в LM Studio с параметрами:
                        *   Context: 32768, GPU Layers: 36, Threads: 4, Batch: 512
                        *   RoPE: Auto, Offload KV: On, mmap: On, Flash Attn: On
                        *   Quant: Q8_0.
                        *   Загрузи лучшую доступную модель. Повтори попытку.
                    *   **Нет:** Завершай работу.
                *   *Нет:* Завершай работу.

### 2. Анализ проблем (Бот-Критик)
*   **Он нашел, к чему придраться?**
    *   **НЕТ:** Запусти режим "прожарки" (тотальная критика). Нашел теперь?
        *   **НЕТ:** Придумай новую фичу (неконфликтующую) -> Реализуй -> Блок 3.
        *   **ДА:** Это его первая придирка к этому элементу?
            *   **НЕТ:** Добавь в системный промпт исключение ("так и должно быть") -> Блок 3.
            *   **ДА:** Исправь это -> Блок 3.
    *   **ДА:** Это его первая придирка?
        *   **НЕТ:** Добавь исключение в промпт -> Блок 3.
        *   **ДА:** Исправь это -> Блок 3.

### 3. Сборка и Проверка
*   **Исправь код / Реализуй фичу.**
*   **Проверь сборку (`./gradlew assembleDebug`).**
    *   **Собралось?**
        *   **НЕТ:** Исправь ошибки сборки.
            *   *Критическая регрессия (1 исправил, 2 сломал)?*
                *   **ДА:** Загрузи последнюю версию с Git (`git restore .`), повтори попытку аккуратнее.
                *   **НЕТ:** Продолжай исправлять.
        *   **ДА:** Запусти `ui_walker.py` (или `run_only` режим) для верификации.
            *   **Завершилось без ошибок?**
                *   **НЕТ:** Вернись к исправлению (Блок 2/3).
                *   **ДА:** Залей на GitHub? (Если требуется).
                    *   **ДА:** Конец задачи.
                    *   **НЕТ:** Конец задачи.

### 4. Аварийные ситуации и Очистка
*   **Закончилось место на диске?**
    *   Очисти кэш Gradle, pip, npm, удали старые AVD, очисти `%TEMP%`.
    *   Очистилось? -> Да: Продолжай. -> Нет: Завершайся.
*   **Тупик (нужна инфо, которой нет)?** -> Завершай работу.
*   **Очистка:** После завершения работы очисти логи и временные файлы.